...
class ExamsGenerator():
    ...
    def export_corrector(self, exam_number):
        solutions = {}
        for index, question in enumerate(self.questions):
            for number_option, option in enumerate(question['options']):
                if option['correct']:
                    solutions[index + 1] = number_option + 1
                    break
        with open(f"exam_{exam_number + 1}_solutions.data", 'w') as f:
